...
void filter_N_v4 (TString file_name)
{
	...
	TString path_in = "/data4/chengwei/Geant4/INTT_simulation/G4/for_CW/Final_BeamTest2021_Publication/data/Run64/";
	//===================================option===========================================
	...
	TFile *file_output = new TFile(path_in+file_name+"_filter_all.root", "RECREATE");
	// file_output->cd();
	...
	for (int i=0; i<nEvent_both; i++) // # of total events
	{
		...
		for (int j=0; j<lenvec; j++)
		{
			if (vec_module->at(j) != module_l0 && vec_module->at(j) != module_l1 && vec_module->at(j) != module_l2) {continue;}
			if (vec_chip_id->at(j) < 1 || vec_chip_id->at(j) > 26) {continue;} // note : range : 1 ~ 26
			if (vec_chan_id->at(j) < 0 || vec_chan_id->at(j) > 127) {continue;} // note : range : 0 ~ 127
			if (vec_adc->at(j) < 0 || vec_adc->at(j) > 7) {continue;} // note : range : 0 ~ 7
			if (vec_bco->at(j) < 0 || vec_bco->at(j) > 127) {continue;} // note : range : 0 ~ 127
			if (vec_bco_full->at(j) < 0 || vec_bco_full->at(j) > 65535) {continue;} // note : range : 0 ~ 65535
			...
			else 
			{
				...
				if (vec_element == 0){
					std::cout<<"------------------parent_eID: "<<parent_eID<<", sub_eID: "<<sub_eID<<", zero hits!"<<std::endl;
				}
				...
				if (sub_eID > 999){
					std::cout<<"!!!!!!! eID re-formating error !!!!!!!!!, in mother event: "<<i<<", sub_eID: "<<sub_eID<<", eID re-formating: "<<eID_element<<std::endl;
				}
