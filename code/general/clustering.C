...
void clustering(int index/*TString file_name*/) // by analyzing the tree_both
{
	...
	for (int i=0; i<8; i++) {adc_convert[i] = adc_setting[i];} // note: 2025/04/21 we now use the threshold to represent the adc value.
	
	//todo : for different runs, change here
	int camac_tdc_id = 3;	
	int camac_tdc_cutL = 0; //the camac_tdc cut,  left-hand side cut, 510 in principle
	int camac_tdc_cutR = 2000;//the camac_tdc cut, right-hand side cut


	bool DSE_on = true;// false -> no dse, true -> include double saving event 
	...
	int  study_chip = 9; // (1 ~ 13) if major_chip_check is false, you have to give a chip_ID that you want to study
	
	//todo : for different runs, change here
	bool camac_adc_on = false; // to use camac_adc cut or not, true -> camac_adc is a cut
	int camac_adc_cut_0L = 0; // the camac_adc cut l0 left
	int camac_adc_cut_0R = 2000; // the camac_adc cut l0 riight

	int camac_adc_cut_1L = 0; // the camac_adc cut l1 left 
	int camac_adc_cut_1R = 2000; // the camac_adc cut l1 right
	
	int camac_adc_cut_2L = 0; // the camac_adc cut l2 left 
	int camac_adc_cut_2R = 2000; // the camac_adc cut l2 right
	
	
	int analysis_size = 30000000; // the # of event you want to study


	int info_sampling = 100; //to print the information per "info_sampling" event.
	...
	TH1D * adc_distribution[3];
	for (int i = 0; i < number_of_layer; i++)
	{
		adc_distribution[i]= new TH1D(Form("h1D_HitAdc_Module_%i",module_ID[i]),Form("h1D_HitAdc_Module_%i;Hit Adc;Entries",module_ID[i]),8,0,8);
	}

	TH1D * h1D_bco_diff = new TH1D("h1D_bco_diff","h1D_bco_diff;Hit_bco - BcoFull&0x7FU;Entries",128,0,128);

	TH2D * h2D_hitmap_l0 = new TH2D(Form("h2D_hitmap_l0"), Form("h2D_hitmap_l0;Chip ID (13 -> 1);Channel ID"),13,-0.5,12.5,256,-0.5,255.5);
	TH2D * h2D_hitmap_l1 = new TH2D(Form("h2D_hitmap_l1"), Form("h2D_hitmap_l1;Chip ID (13 -> 1);Channel ID"),13,-0.5,12.5,256,-0.5,255.5);
	TH2D * h2D_hitmap_l2 = new TH2D(Form("h2D_hitmap_l2"), Form("h2D_hitmap_l2;Chip ID (13 -> 1);Channel ID"),13,-0.5,12.5,256,-0.5,255.5);
	
	// vector<TH2D *> h2D_ClusPos; h2D_ClusPos.clear();
	// h2D_ClusPos.push_back(new TH2D(Form("h2D_ClusPos_l0"), Form("h2D_ClusPos_l0;Chip ID (13 -> 1);Y axis [cm]"),13,-0.5,12.5,220,-11, 11));
	// h2D_ClusPos.push_back(new TH2D(Form("h2D_ClusPos_l1"), Form("h2D_ClusPos_l1;Chip ID (13 -> 1);Y axis [cm]"),13,-0.5,12.5,220,-11, 11));
	// h2D_ClusPos.push_back(new TH2D(Form("h2D_ClusPos_l2"), Form("h2D_ClusPos_l2;Chip ID (13 -> 1);Y axis [cm]"),13,-0.5,12.5,220,-11, 11));

	// cout<<h2D_ClusPos[0]<<endl;
	// cout<<h2D_ClusPos[1]<<endl;
	// cout<<h2D_ClusPos[2]<<endl;
	...
	vector<int> DSE_array; DSE_array.clear();
	...
	vector<int> * bco = new vector<int>();
	vector<int> * bco_full = new vector<int>();
	...
	for (int i=0; i<event_N; i++)
	{
		...
		// if ( camac_tdc->at(camac_tdc_id)>camac_tdc_cutL && camac_tdc->at(camac_tdc_id)<camac_tdc_cutR) //INTT_event, camac_tdc range cut
		if ( true )
		{

			//todo : for different runs (data/MC), change here
			// if (camac_adc_on == true ) // true -> the consider camac_adc as a cut as well
			if ( false ) // true -> the consider camac_adc as a cut as well
			{
			...
			for (int i1=0; i1<event_length; i1++)
			{
				...
				{
					int bco_diff = (bco->at(i1) - (bco_full->at(i1) & 0x7fU) + 128) % 128;
					h1D_bco_diff->Fill(bco_diff);
					...
					if      (module->at(i1) == module_ID[0])
					{
						...
						adc_distribution[0]->Fill(adc->at(i1));

						h2D_hitmap_l0->Fill(convert_chipID, convert_chanID);
					...
					else if (module->at(i1) == module_ID[1])
					{
						...
						adc_distribution[1]->Fill(adc->at(i1));

						h2D_hitmap_l1->Fill(convert_chipID, convert_chanID);
					}
					else if (module->at(i1) == module_ID[2])
					{
						...
						adc_distribution[2]->Fill(adc->at(i1));

						h2D_hitmap_l2->Fill(convert_chipID, convert_chanID);
					}
					...
					// 	adc_distribution[3]->Fill(adc->at(i1));
					// }	
				}
				else
				{
			...
			DSE_array.push_back(DSE); // to save the DSE of the event pass all cuts
	...
	{
		vector<TH2D *> h2D_ClusPos; h2D_ClusPos.clear();
		h2D_ClusPos.push_back(new TH2D(Form("h2D_ClusPos_l0"), Form("h2D_ClusPos_l0;Chip ID (13 -> 1);Y axis [cm]"),13,-0.5,12.5,128,-9.984,9.984));
		h2D_ClusPos.push_back(new TH2D(Form("h2D_ClusPos_l1"), Form("h2D_ClusPos_l1;Chip ID (13 -> 1);Y axis [cm]"),13,-0.5,12.5,128,-9.984,9.984));
		h2D_ClusPos.push_back(new TH2D(Form("h2D_ClusPos_l2"), Form("h2D_ClusPos_l2;Chip ID (13 -> 1);Y axis [cm]"),13,-0.5,12.5,128,-9.984,9.984));

		vector<TH1D *> h1D_ClusPhiSize; h1D_ClusPhiSize.clear();
		h1D_ClusPhiSize.push_back(new TH1D("h1D_ClusPhiSize_l0", "h1D_ClusPhiSize_l0;Cluster Phi Size;Entries", 30, 0, 30));
		h1D_ClusPhiSize.push_back(new TH1D("h1D_ClusPhiSize_l1", "h1D_ClusPhiSize_l1;Cluster Phi Size;Entries", 30, 0, 30));
		h1D_ClusPhiSize.push_back(new TH1D("h1D_ClusPhiSize_l2", "h1D_ClusPhiSize_l2;Cluster Phi Size;Entries", 30, 0, 30));

		vector<TH1D *> h1D_ClusAdc; h1D_ClusAdc.clear();
		h1D_ClusAdc.push_back(new TH1D("h1D_ClusAdc_l0", "h1D_ClusAdc_l0;Cluster Adc;Entries", 100, 0, 3000));
		h1D_ClusAdc.push_back(new TH1D("h1D_ClusAdc_l1", "h1D_ClusAdc_l1;Cluster Adc;Entries", 100, 0, 3000));
		h1D_ClusAdc.push_back(new TH1D("h1D_ClusAdc_l2", "h1D_ClusAdc_l2;Cluster Adc;Entries", 100, 0, 3000));

		vector<TH2D *> h2D_ClusPhiSize_ClusAdc; h2D_ClusPhiSize_ClusAdc.clear();
		h2D_ClusPhiSize_ClusAdc.push_back(new TH2D("h2D_ClusPhiSize_ClusAdc_l0", "h2D_ClusPhiSize_ClusAdc_l0;Cluster Phi Size;ClusAdc", 30,0, 30, 100, 0, 3000));
		h2D_ClusPhiSize_ClusAdc.push_back(new TH2D("h2D_ClusPhiSize_ClusAdc_l1", "h2D_ClusPhiSize_ClusAdc_l1;Cluster Phi Size;ClusAdc", 30,0, 30, 100, 0, 3000));
		h2D_ClusPhiSize_ClusAdc.push_back(new TH2D("h2D_ClusPhiSize_ClusAdc_l2", "h2D_ClusPhiSize_ClusAdc_l2;Cluster Phi Size;ClusAdc", 30,0, 30, 100, 0, 3000));
		...
		cluster_info_output_tree -> Branch("DSE",&cluster_info_DSE);
		...
		for (int i=0; i<event_event.size(); i++)
		{
			...
			{
				for (int i2=0; i2<event_event[i][i1].size(); i2++) // note : the # of hit of this layer of this event
				{
				...
				if ( data_all[0][i10].size() + data_all[1][i10].size() + data_all[2][i10].size() + data_all[3][i10].size() > 0)
				{
					...
					for (int i11 = 0; i11 < 4; i11++) // note : layer
					{
						for (int i12 = 0; i12 < data_all_MC_EW_chip[i10][i11].size(); i12 ++) // note : also check the # of hit
						{
							...
							// cout<<"cluster info : "<<i11<<", "<<i10<<", "<<cluster_info_position.back()<<endl;
							// cout<<h2D_ClusPos[2]<<endl;

							int convert_chipID = -(i10+1)+ (1 + int((i10+1) / 14)) * 13;
							h2D_ClusPos[i11] -> Fill(convert_chipID, cluster_info_position.back());

							h1D_ClusPhiSize[i11] -> Fill(cluster_info_hit.back());
							h1D_ClusAdc[i11] -> Fill(cluster_info_adc.back());
							h2D_ClusPhiSize_ClusAdc[i11] -> Fill(cluster_info_hit.back(), cluster_info_adc.back());
		...
		h1D_bco_diff -> Write("", TObject::kOverwrite);

		h2D_hitmap_l0 -> Write("", TObject::kOverwrite);
		h2D_hitmap_l1 -> Write("", TObject::kOverwrite);
		h2D_hitmap_l2 -> Write("", TObject::kOverwrite);

		h2D_ClusPos[0] -> Write("", TObject::kOverwrite);
		h2D_ClusPos[1] -> Write("", TObject::kOverwrite);
		h2D_ClusPos[2] -> Write("", TObject::kOverwrite);

		h1D_ClusPhiSize[0] -> Write("", TObject::kOverwrite);
		h1D_ClusPhiSize[1] -> Write("", TObject::kOverwrite);
		h1D_ClusPhiSize[2] -> Write("", TObject::kOverwrite);

		h1D_ClusAdc[0] -> Write("", TObject::kOverwrite);
		h1D_ClusAdc[1] -> Write("", TObject::kOverwrite);
		h1D_ClusAdc[2] -> Write("", TObject::kOverwrite);

		h2D_ClusPhiSize_ClusAdc[0] -> Write("", TObject::kOverwrite);
		h2D_ClusPhiSize_ClusAdc[1] -> Write("", TObject::kOverwrite);
		h2D_ClusPhiSize_ClusAdc[2] -> Write("", TObject::kOverwrite);


		for (int li = 0; li < 3; li++)
		{
			adc_distribution[li] -> Write("", TObject::kOverwrite);
		}

		cluster_info_output_file -> Close();

		cout<<"clustering done, close the file"<<endl;
