...
double BeamSpot_region_High = 6.0;
double BeamSpot_region_Low = -7.0;
...
VariationSet MakeEffiPos(std::vector<std::string> input_file_vec) {
    ...
    for (int i = 1; i <= N_bins_pos_hist; i++) {
        ...
        if (bin_center > BeamSpot_region_Low && bin_center < BeamSpot_region_High) {
...
RelativeVariationSet GetVariation (
    ...
    TPad * pad1 = new TPad("", "", 0, 0.31, 1, 0.85);
    pad1 -> Draw();
    pad1 -> cd();
    ...
    for (int i = 0; i < VariationSet_vec.size(); i++) {
        if (i == 0) {
            ...
            for (int j = 0; j < VariationSet_vec[i].effi_pos_gr -> GetN(); j++) {
                ...
                else if (y < MaxVariationDown->GetPointY(j)) {
    ...
    std::cout<<"MaxVariationUp middle: ";
    for (int i = 0; i < MaxVariationUp->GetN(); i++) {
        std::cout<<Form("%.5f", MaxVariationUp->GetPointY(i))<<", ";
    }
    std::cout<<std::endl;

    std::cout<<"MaxVariationDown middle: ";
...
std::pair<double,double> CombineSyst(
    ...
    TLegend * leg_errors_Final = new TLegend(0.21,0.29,0.41,0.43);
    ...
    for (int i = 0; i < effi_pos_gr_syst -> GetN(); i++) {
        ...
        y_error_up = (y_error_up > 100) ? 100 : y_error_up;
        y_error_down = (y_error_down > 100) ? 100 : y_error_down;
...
int FinalResult() {
    ...
    std::cout<<"Statistical Uncertainty : "<<BaseLine_EffiSet.all_Final_Effi_StatErrorUp<<" - "<<BaseLine_EffiSet.all_Final_Effi_StatErrorDown<<std::endl;
    std::cout<<"Systematic Uncertainty  : "<<all_syst_unc_pair.first * BaseLine_EffiSet.all_Final_Effi<<" - "<<all_syst_unc_pair.second * BaseLine_EffiSet.all_Final_Effi<<std::endl;
