...
void Characterize_Rate1D (TH1D *hist,  int hcolour, pair<double,double> Y_range_pair={0.4,3.2})
{
	float ratio = 7.0/3.0;
	hist -> SetTitle       ("");
	...
	hist -> GetYaxis() -> SetRangeUser   (Y_range_pair.first, Y_range_pair.second);
...
// note : titles_vec[0] : x
// note : titles_vec[1] : y
void dataMC_comp (TH1D* hist_data, TH1D* hist_MC, TString folder_direction, TString plot_name, vector<TString> titles_vec, pair<double, double> ratio_Y_range_pair = {0.4, 3.2} ,bool linear_or_log = false, bool statsbox_bool = false)
{
    ...
    double Y_axis_max = (linear_or_log) ? 500 : 2.0;
    ...
    Characterize_Rate1D(hist_ratio,1, ratio_Y_range_pair);
...
void binbin_uniform(TH1D * input_hist, vector<double>binbin_factor)
{
...
// note : titles_vec[0] : x
// note : titles_vec[1] : y
void multi_plot_comp (vector<TH1D*>input_hist_1D, vector<TString>hist_color_array, vector<TString>Legend_array, TString folder_direction, TString plot_name, vector<TString> titles_vec ,bool set_logY = false, bool statsbox_bool = false)
{
    TCanvas * c3 = new TCanvas("c3","c3",850,800);
    c3 -> cd();
    ...
    {
        Characterize_Hist1D(input_hist_1D[i],0,hist_color_array[i],set_logY,false,"",titles_vec[0],titles_vec[1],false,true);    
    ...
    // Characterize_Hist1D(hist_data,0,"#1A3947",set_logY);
    // hist_data -> GetXaxis() -> SetTitle(titles_vec[0]);
    // hist_data -> GetYaxis() -> SetTitle(titles_vec[1]);
    // // hist_data -> GetYaxis() -> SetRangeUser(0,1);

    // Characterize_Hist1D(hist_MC,0,"#A08144",set_logY);
    // hist_MC -> GetXaxis() -> SetTitle(titles_vec[0]);
    // hist_MC -> GetYaxis() -> SetTitle(titles_vec[1]);
    // // hist_MC -> SetTitle(titles_vec[2]);
    // // hist_MC -> GetYaxis() -> SetRangeUser(0,1);
